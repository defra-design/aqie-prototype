...
import * as nunjucks from 'nunjucks/browser/nunjucks.js'
import './templates.js'
...
export function MapContainer (mapId, options) {
  // Setup defaults
  const defaults = {
    minIconResolution: window.maps.minResolution,
    keyTemplate: '',
    controls: []
  }
  ...
  document.documentElement.classList.add('app-map-template')
  document.body.classList.add('app-map-template__body')

  // Create the map container element
  const containerElement = document.createElement('div')
  containerElement.id = mapId
  containerElement.className = `app-map ${options.class}`
  ...
  mapLabel.className = 'app-map__title'
  ...
  viewport.className = `app-map__viewport ${viewport.className}`
  viewport.tabIndex = 0
  ...
  controlsElement.className = 'app-map-controls'
  ...
  exitMapButtonElement.className = 'app-map__exit'
  ...
  openKeyButtonElement.className = 'app-map__open-key'
  ...
  controlsBottomElement.className = 'app-map-controls__bottom'
  controlsElement.appendChild(controlsBottomElement)

  // Create reset control
  const resetButtonElement = document.createElement('button')
  resetButtonElement.className = 'app-map-reset'
  ...
  infoElement.className = 'app-map-info'
  infoElement.id = 'info'
  ...
  closeInfoButton.className = 'app-map-info__close'
  ...
  infoContainer.className = 'app-map-info__container'

  const infoContent = document.createElement('div')
  infoContent.id = 'infoContent'
  infoContent.className = 'app-map-info__content'
  ...
  closeKeyButton.className = 'app-map-key__close'
  ...
  keyContent.className = 'app-map-key__content'
  keyContent.innerHTML = nunjucks.render(options.keyTemplate)
  ...
  const mapSiblings = document.querySelectorAll('body > *:not(.app-map):not(script):not([aria-hidden="true"])')
  mapSiblings.forEach(mapSibling => {
    mapSibling.setAttribute('aria-hidden', 'true')
    mapSibling.classList.add('app-map-!-visibility-hidden')
  })
  ...
  const hideSiblings = (isHidden) => {
    ...
    siblings.forEach(sibling => {
      sibling.setAttribute('aria-hidden', isHidden)
    })
  }
  ...
  const removeContainer = () => {
    ...
    // Unlock body scroll
    document.body.classList.remove('app-map-template__body')
    document.documentElement.classList.remove('app-map-template')
    clearAllBodyScrollLocks()
    // Re-instate aria-hidden elements
    mapSiblings.forEach(mapSibling => {
      mapSibling.removeAttribute('aria-hidden')
      mapSibling.classList.remove('app-map-!-visibility-hidden')
    })
    // Remove DOM elements
  ...
  const openKey = () => {
    ...
    containerElement.classList.add('app-map--key-open')
  ...
  const closeKey = () => {
    ...
    containerElement.classList.remove('app-map--key-open')
    if (state.isTablet) {
  ...
  const closeInfo = () => {
    ...
    containerElement.classList.remove('app-map--info-open')
    // Re-enable screen reader access to siblings
    hideSiblings(false)
    keyElement.setAttribute('aria-hidden', state.isTablet)
    state.isAttributionsOpen ? attributionElement.focus() : viewport.focus()
  ...
  this.attributionButton = attributionElement
  this.state = state
  ...
  this.showInfo = (title, body) => {
    ...
    containerElement.classList.add('app-map--info-open')
  ...
  const zoomButtons = document.querySelectorAll('.app-map-zoom button')
  ...
  const tabletListener = (tabletMediaQuery) => {
    ...
    state.isKeyOpen = (containerElement.classList.contains('app-map--key-open') && state.isTablet) || !state.isTablet
  ...
  attributionElement.addEventListener('click', (e) => {
  ...
  const keydown = (e) => {
    ...
    if (e.key === 'Tab') {
      ...
      if (tabring.classList.contains('app-map')) {
        specificity = `#${tabring.id} div[role="application"]` + ','
        specificity += selectors.map(i => `#${tabring.id} > .app-map-controls ${i}`).join(',') + ','
      ...
      // VoiceOver verbosity issues?? Remove info label and description
      if (state.isInfoOpen) {
      ...
      const regionSelector = '.app-map div[role="dialog"][open="true"], .app-map div[role="region"], .app-map div[role="application"]'
